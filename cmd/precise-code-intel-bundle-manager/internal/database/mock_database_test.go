...
func NewMockDatabase() *MockDatabase {
	return &MockDatabase{
		...
		DiagnosticsFunc: &DatabaseDiagnosticsFunc{
			defaultHook: func(context.Context, string) ([]Diagnostic, error) {
				return nil, nil
			},
		},
		ExistsFunc: &DatabaseExistsFunc{
...
type DatabaseDiagnosticsFunc struct {
	defaultHook func(context.Context, string) ([]Diagnostic, error)
	hooks       []func(context.Context, string) ([]Diagnostic, error)
...
// the parameter and result values of this invocation.
func (m *MockDatabase) Diagnostics(v0 context.Context, v1 string) ([]Diagnostic, error) {
...
// of the parent MockDatabase instance is invoked and the hook queue is
// empty.
func (f *DatabaseDiagnosticsFunc) SetDefaultHook(hook func(context.Context, string) ([]Diagnostic, error)) {
	f.defaultHook = hook
}
...
// at the front of the queue and discards it. After the queue is empty, the
// default hook function is invoked for any future action.
func (f *DatabaseDiagnosticsFunc) PushHook(hook func(context.Context, string) ([]Diagnostic, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
// SetDefaultReturn calls SetDefaultDefaultHook with a function that returns
// the given values.
func (f *DatabaseDiagnosticsFunc) SetDefaultReturn(r0 []Diagnostic, r1 error) {
	f.SetDefaultHook(func(context.Context, string) ([]Diagnostic, error) {
		return r0, r1
	})
}
...
// PushReturn calls PushDefaultHook with a function that returns the given
// values.
func (f *DatabaseDiagnosticsFunc) PushReturn(r0 []Diagnostic, r1 error) {
	f.PushHook(func(context.Context, string) ([]Diagnostic, error) {
		return r0, r1
	})
}

func (f *DatabaseDiagnosticsFunc) nextHook() func(context.Context, string) ([]Diagnostic, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
